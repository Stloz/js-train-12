...
function getFilteredDictionarySize(dictionary, filter) {
  ...
  for (const [key, value] of dictionary.entries()) {
    if (filter(key, value)) filteredMap.set(key, value);
  }
...
function convertDictionaryToSet(dictionary) {
  ...
  dictionary.forEach((value, key) => {
    if (typeof key === "number") set.add(key + 1);
    else if (typeof key === "string") set.add(key.toUpperCase());
  });
  return set;
}
...
function convertSetToDictionary(set) {
  ...
  const map = new Map();
  set.forEach((element) => map.set(element, element.charCodeAt(0)));
  //   set.forEach((element) => map.set(element, element.codePointAt(0)));
  return map;
}
